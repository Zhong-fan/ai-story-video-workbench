...
| 视频生成 | 即梦 AI 3.0 文生视频 / 图生视频-首帧 + 本地 FFmpeg 拼接 |
...
- 批量正文任务的结构化进度摘要：
  ...
  - `total_chapters / completed_chapters / failed_chapters / current_chapter_no / last_updated_at`
- 分镜任务的结构化进度摘要：
  - `storyboard.progress`
  - `source_chapter_count / shot_count / last_event_message / preflight_summary`
- 视频任务的结构化进度摘要：
  - `video_tasks.progress_json`
  - `current_shot_no / current_step / completed_shot_count / failure_stage / last_updated_at`
- 镜头级进度：
  - `progress.shots[*].image_status / dialogue_status / subtitle_status / composed_status`
  - `progress.shots[*].used_first_frame`

## 锁定与不覆盖

当前主链路已经开始明确尊重用户确认结果，后续生成不会默认覆盖以下内容：

- 已锁定的长篇概要与章节概要
- 已锁定的角色三视图 `character_turnaround`
- 已锁定的镜头首帧 `shot_first_frame`
- 已锁定的自动对白脚本 `audio_script.audio_script_locked`
- 已锁定的对白/旁白素材 `media_assets.meta.locked`

视频生产预检完成后，会通过真实事件回写这次“生成了什么 / 跳过了什么”：

- `storyboard_preflight_completed`
...
如果没有可用首帧，系统会自动回退到原有文生视频路径。

## 镜头约束

当前镜头生成不再只依赖自由文本 `visual_prompt`。后端会把以下约束真实叠加进视觉 prompt：
...
- 手写迁移没有 rollback。
- 视频预览和下载体验还比较基础。
- 即梦图生视频目前先接“首帧”分支，首尾帧、运镜参数等能力尚未接入。
- `character_refs / scene_refs` 已进入视觉 prompt，但前端镜头级资产选择器仍是最小可用形态。
- 最终视频拼接仍依赖 FFmpeg。
